...
extern __interrupt void MainCtrlISR(void);
...
#define MTR1_IU_ADC_SOC_NUM     ADC_SOC_NUMBER2         // ADCA-A11*/C10-SOC1-PPB1
#define MTR1_IV_ADC_SOC_NUM     ADC_SOC_NUMBER2         // ADCC-A14/C4* -SOC1-PPB1
#define MTR1_IW_ADC_SOC_NUM     ADC_SOC_NUMBER4         // ADCC-A15/C7* -SOC2-PPB2
...
#define MTR2_IU_ADC_SOC_NUM     ADC_SOC_NUMBER8         // ADCA-A10*/C10-SOC7-PPB3
#define MTR2_IV_ADC_SOC_NUM     ADC_SOC_NUMBER8         // ADCC-A9/C8*  -SOC7-PPB3
#define MTR2_IW_ADC_SOC_NUM     ADC_SOC_NUMBER10        // ADCC-A4/C14* -SOC8-PPB4
#define MTR2_VU_ADC_SOC_NUM     ADC_SOC_NUMBER10        // ADCA-A5*/C2  -SOC10
#define MTR2_VV_ADC_SOC_NUM     ADC_SOC_NUMBER11        // ADCA_A7*/C3  -SOC11
#define MTR2_VW_ADC_SOC_NUM     ADC_SOC_NUMBER12        // ADCC_A8/C11* -SOC11
#define MTR2_VDC_ADC_SOC_NUM    ADC_SOC_NUMBER12        // ADCA-A1*     -SOC12
...
#define MTR2_ADC_INT_BASE       ADCA_BASE               // ADCC-C6  -SOC6
#define MTR2_ADC_INT_NUM        ADC_INT_NUMBER1         // ADCC_INT1-SOC6
#define MTR2_ADC_INT_SOC        ADC_SOC_NUMBER12        // ADCC_INT1-SOC6

#define MTR2_PIE_INT_NUM        INT_ADCA1               // ADCC_INT1-SOC6
...
#define HAL_ackADCInt HAL_ackMtr1ADCInt
static __UNUSED ALWAYSINLINE void HAL_ackADCInt(void)
{
    // clear the ADC interrupt flag
    ADC_clearInterruptStatus(MTR1_ADC_INT_BASE, MTR1_ADC_INT_NUM);
    ADC_clearInterruptStatus(MTR2_ADC_INT_BASE, MTR2_ADC_INT_NUM);

    // Acknowledge interrupt from PIE group
    Interrupt_clearACKGroup(MTR1_INT_ACK_GROUP);
    Interrupt_clearACKGroup(MTR2_INT_ACK_GROUP);
...
//! \param[in]  handle  The hardware abstraction layer (HAL) handle
extern void HAL_setupMtrFaults(HAL_MTR_Handle handle, const MotorNum_e motorNum);
...
extern HAL_MTR_Handle HAL_MTR2_init(void *pMemory, const size_t numBytes);
...
{
    // Motor_1->ADCA_INT1
    Interrupt_register(MTR1_PIE_INT_NUM, &MainCtrlISR);
...
static __UNUSED ALWAYSINLINE void HAL_readADCData(HAL_ADCData_t *pADCData , const MotorNum_e motorNum)
{
    float32_t value;
    ...
    {
		// convert phase A current
		value = (float32_t)ADC_readResult(MTR2_IU_ADCRES_BASE, MTR2_IU_ADC_SOC_NUM);
		pADCData->I_A.value[0] = value * pADCData->current_sf;

		// convert phase B current
		value = (float32_t)ADC_readResult(MTR2_IV_ADCRES_BASE, MTR2_IV_ADC_SOC_NUM);
		pADCData->I_A.value[1] = value * pADCData->current_sf;

		// convert phase C current
		value = (float32_t)ADC_readResult(MTR2_IW_ADCRES_BASE, MTR2_IW_ADC_SOC_NUM);
		pADCData->I_A.value[2] = value * pADCData->current_sf;

		// convert phase A voltage
		value = (float32_t)ADC_readResult(MTR2_VU_ADCRES_BASE, MTR2_VU_ADC_SOC_NUM);
		pADCData->V_V.value[0] = value * pADCData->voltage_sf;

		// convert phase B voltage
		value = (float32_t)ADC_readResult(MTR2_VV_ADCRES_BASE, MTR2_VV_ADC_SOC_NUM);
		pADCData->V_V.value[1] = value * pADCData->voltage_sf;

		// convert phase C voltage
		value = (float32_t)ADC_readResult(MTR2_VW_ADCRES_BASE, MTR2_VW_ADC_SOC_NUM);
		pADCData->V_V.value[2] = value * pADCData->voltage_sf;
		pADCData->V_V.value[2] = value * pADCData->voltage_sf;

		// convert dc bus voltage
		value = (float32_t)ADC_readResult(MTR2_VDC_ADCRES_BASE, MTR2_VDC_ADC_SOC_NUM);
		pADCData->VdcBus_V = value * pADCData->dcBusvoltage_sf;
    }

    return;
} // end of HAL_readADCData() functions
...
//!             offset values for voltage and current measurements.
//! \param[in]  handle       The hardware abstraction layer (HAL) handle
extern void HAL_MTR_setParams(HAL_MTR_Handle handle, USER_Params *pUserParams, const MotorNum_e motorNum);
...
//! \param[in]  handle  The hardware abstraction layer (HAL) handle
extern void HAL_setupCMPSSs(HAL_MTR_Handle handle, const MotorNum_e motorNum);
...
//! \param[in] handle       The hardware abstraction layer (HAL) handle
extern void HAL_setupGate(HAL_MTR_Handle handle, const MotorNum_e motorNum);
...
//        pPWMData->cmpValue[pwmCnt]  = (int16_t)(V_sat_dc_pu * period/20) + (period-period/20)/2;  //
...
//! \param[in] handle          The hardware abstraction layer (HAL) handle
extern void HAL_setupPWMs(HAL_MTR_Handle handle, const MotorNum_e motorNum);
