...
AdcTrim_cal(void)
{
	Uint32 temp = HWREG(CPUSYS_BASE + SYSCTL_O_PCLKCR13);
	Uint32 switchData = 0;
	Uint32 trimData = 0;

	EALLOW;
	HWREG(CPUSYS_BASE + SYSCTL_O_PCLKCR13) = 0x5;

	switchData = HWREG(0x7A0864);
	if(0x5A5A == (switchData >> 16))
	{
		trimData = switchData & 0xFFFF;
		HWREG(ADCA_BASE + 0x9CU) = trimData;
	}

	switchData = HWREG(0x7A086C);
	if(0x5A5A == (switchData >> 16))
	{
		trimData = switchData & 0xFFFF;
		HWREG(ADCC_BASE + 0x9CU) = trimData;
	}

	switchData = HWREG(0x7A0874);
	trimData = HWREG(0x7A0878);
	if(0x5A5A == (switchData >> 16))
	{
		HWREG(ADCA_BASE + 0x190U) = trimData;
	}

	HWREG(CPUSYS_BASE + SYSCTL_O_PCLKCR13) = temp;
	EDIS;
}
//*****************************************************************************
//
