...
//
// Define to pass to SysCtl_setClock(). Will configure the clock as follows:
// PLLSYSCLK = 20MHz (SYSCTL_OSCSRC_OSC2) * 16 (IMULT) / (2 (REFDIV) * 1 (ODIV) * 1(SYSDIV))
//

#define DEVICE_SETCLOCK_CFG          (SYSCTL_OSCSRC_XTAL| SYSCTL_IMULT(16) | \
                                      SYSCTL_REFDIV(2) | SYSCTL_ODIV(1) |SYSCTL_VCO(2) |\
                                      SYSCTL_SYSDIV(1) | SYSCTL_PLL_ENABLE | \
                                      SYSCTL_DCC_BASE_0)
...
// code below if a different clock configuration is used!
//
#define DEVICE_SYSCLK_FREQ          ((DEVICE_OSCSRC_FREQ * 16) / (2 * 1* 1))
#endif
//
